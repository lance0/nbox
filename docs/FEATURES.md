...
| `nbox ip <addr> [--vrf] [--journal]` | IP + most-specific parent prefix (VRF-scoped) and its VLAN plus the prefix's `scope`/`scope_type` (site, location, region, …); surfaces `nat_inside`/`nat_outside` (NetBox 4.6) when set. `reserve <prefix> [--description] [--dns-name] [--count N]` is a safe write (ADR-0001): `POST` to `available-ips`, behind `--allow-writes` + confirm. `--count N` allocates N IPs (sequential POSTs); partial failure exits 1. `--dry-run` previews the candidate; the receipt's `object` is the created IP (or array for `--count > 1`). |
...
| `nbox ip-range <start\|id>` | IP range. `reserve [--description] [--dns-name] [--count N]` is a safe write (ADR-0001): `POST` to `available-ips`, behind `--allow-writes` + confirm. `--count N` allocates N IPs (sequential POSTs); partial failure exits 1. `--dry-run` previews the candidate; the receipt's `object` is the created IP (or array for `--count > 1`). |
